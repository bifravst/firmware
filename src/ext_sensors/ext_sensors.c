...
int ext_sensors_init(ext_sensors_evt_handler_t handler)
{
	...
	if(temp_sensor.dev == NULL) {
		LOG_ERR("Could not get device binding %s", temp_sensor.dev_name);
		return -ENODATA;
	}

	humid_sensor.dev = device_get_binding(humid_sensor.dev_name);
	if(humid_sensor.dev == NULL) {
		LOG_ERR("Could not get device binding %s", humid_sensor.dev_name);
		return -ENODATA;
	}

	accel_sensor.dev = device_get_binding(accel_sensor.dev_name);
	if(accel_sensor.dev == NULL) {
		LOG_ERR("Could not get device binding %s", accel_sensor.dev_name);
		return -ENODATA;
	}
	...
	if (IS_ENABLED(CONFIG_ACCELEROMETER_TRIGGER)) {
		...
		if (sensor_trigger_set(accel_sensor.dev, &trig, accelerometer_trigger_handler)) {
			LOG_ERR("Could not set trigger for device %s", accel_sensor.dev_name);
			return -ENODATA;
		}
	}
